...
const AllRooms = () => {
    ...
    //Handles Changes for filters and sorting
  ...
  return (
    ...
    const filterDestination = (room) =>{
      const destinaton = searchParams.get('destination')
      if(!destinaton) return true
      return room.hotel.city.toLowerCase().includes(destinaton.toLowerCase())
    }
            ...
            <div className='md:w-1/2 flex flex-col gap-2'>
                ...
                <div className='flex flex-wrap item-center mt-3 mb-6 gap-4'>
                    {room.amenities.map((item, index) => (
                      <div key={index} className='flex item-center gap-2 px-3 py-2 rounded-lg bg-[#F5F5F5]/70'>
      ...
      <div className={`${openFilters ? 'h-auto' : 'h-0 lg:h-auto'} overflow-hidden transition-all duration-700`}>
        ...
        <div className='px-5 pt-5'>
          ...
          {priceRanges.map((range, index) => (
            <CheckBox key={index} label={`${currency} ${range}`} selected={selectedFilters.priceRange.includes(range)} onchange={(checked)=>
